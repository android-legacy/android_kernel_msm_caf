...
#define LCDC_ILI9486L_PANEL_NAME		"lcdc_trebon_hvga"
...
static int __devinit trebon_disp_probe(struct platform_device *pdev)
{
	...
	if (pdev->id == 0) {
		...
		spi_sdo = *(lcdc_trebon_pdata->gpio_num + 2); 
		spi_sdi  = *(lcdc_trebon_pdata->gpio_num + 3);
