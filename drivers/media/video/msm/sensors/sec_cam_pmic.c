...
void cam_ldo_power_on_1(void)
{
	...
	ret = gpio_request(GPIO_3M_CAM_STBY, "jena_stby");
	ret = gpio_request(GPIO_3M_CAM_RESET, "jena_reset");
	ret = gpio_request(GPIO_3M_CAM_A_EN, "jena_a_en");
#ifdef CONFIG_S5K4ECGX
	ret = gpio_request(GPIO_3M_CAM_C_EN, "jena_c_en");
	ret = gpio_request(GPIO_3M_CAM_AF_EN, "jena_af_en");
#endif
	gpio_direction_output(GPIO_3M_CAM_STBY, 0);
	gpio_direction_output(GPIO_3M_CAM_RESET, 0);
	gpio_direction_output(GPIO_3M_CAM_A_EN, 0);
#ifdef CONFIG_S5K4ECGX
	gpio_direction_output(GPIO_3M_CAM_C_EN, 0);
	gpio_direction_output(GPIO_3M_CAM_AF_EN, 0);
#endif

	gpio_set_value_cansleep(GPIO_3M_CAM_STBY, 0);		//STBYN :: MSM_GPIO 96
	gpio_set_value_cansleep(GPIO_3M_CAM_RESET, 0);	//RSTN :: MSM_GPIO 85
	gpio_set_value_cansleep(GPIO_3M_CAM_A_EN, 0);
#ifdef CONFIG_S5K4ECGX
	gpio_set_value_cansleep(GPIO_3M_CAM_C_EN, 0);
	msleep(1);

//	printk("GPIO_3M_CAM_C_EN = %d , CAM_AF_EN = %d \n", gpio_get_value(GPIO_3M_CAM_C_EN), gpio_get_value(GPIO_3M_CAM_AF_EN));
	gpio_set_value(GPIO_3M_CAM_C_EN, 1);
	gpio_set_value(GPIO_3M_CAM_AF_EN, 1);
	msleep(1);
//	printk("GPIO_3M_CAM_C_EN = %d , CAM_AF_EN = %d \n", gpio_get_value(GPIO_3M_CAM_C_EN), gpio_get_value(GPIO_3M_CAM_AF_EN));
#endif

//	printk("GPIO_3M_CAM_A_EN = %d\n", gpio_get_value(GPIO_3M_CAM_A_EN));

	gpio_set_value(GPIO_3M_CAM_A_EN, 1);
//	printk("GPIO_3M_CAM_A_EN = %d\n", gpio_get_value(GPIO_3M_CAM_A_EN));
...
//	printk("GPIO_3M_CAM_STBY = %d\n", gpio_get_value(GPIO_3M_CAM_STBY));
	msleep(1);
	gpio_set_value_cansleep(GPIO_3M_CAM_STBY, 1);		//STBYN :: MSM_GPIO 96
	msleep(1);
//	printk("GPIO_3M_CAM_STBY = %d\n", gpio_get_value(GPIO_3M_CAM_STBY));
//	printk("GPIO_3M_CAM_RESET = %d\n", gpio_get_value(GPIO_3M_CAM_RESET));
	gpio_set_value_cansleep(GPIO_3M_CAM_RESET, 1);	//RSTN :: MSM_GPIO 85
	msleep(1);
//	printk("GPIO_3M_CAM_RESET = %d\n", gpio_get_value(GPIO_3M_CAM_RESET));	
}
...
#ifdef CONFIG_S5K4ECGX
	gpio_set_value_cansleep(GPIO_3M_CAM_C_EN, 0);
	gpio_set_value_cansleep(GPIO_3M_CAM_AF_EN, 0);
#endif
	msleep(10);
#ifdef CONFIG_S5K4ECGX
	gpio_free(GPIO_3M_CAM_C_EN);
	gpio_free(GPIO_3M_CAM_AF_EN);
#endif
	gpio_free(GPIO_3M_CAM_A_EN);
	gpio_free(GPIO_3M_CAM_STBY);
	gpio_free(GPIO_3M_CAM_RESET);
}
#endif
...
void cam_flash_main_on(int flashMode)
{
	...
	ret = gpio_request(GPIO_3M_CAM_FLASH_EN, "jena_flash_en");
	ret = gpio_request(GPIO_3M_CAM_FLASH_SET, "jena_flash_set");
	printk("GPIO_3M_CAM_FLASH_EN = %d\n", gpio_get_value(GPIO_3M_CAM_FLASH_EN));
	printk("GPIO_3M_CAM_FLASH_SET = %d\n", gpio_get_value(GPIO_3M_CAM_FLASH_SET));
	
	gpio_set_value_cansleep(GPIO_3M_CAM_FLASH_SET, 0);
	int i;
	for(i=0;i<15;i++)
	{
		gpio_set_value_cansleep(GPIO_3M_CAM_FLASH_EN, 1);
		mdelay(1);
		gpio_set_value_cansleep(GPIO_3M_CAM_FLASH_EN, 0);
		printk("count %d \n",i);
	}
	gpio_set_value_cansleep(GPIO_3M_CAM_FLASH_EN, 1);
	gpio_set_value_cansleep(GPIO_3M_CAM_FLASH_SET, 0);
...
void cam_flash_off(int flashMode)
{
	...
	gpio_free(GPIO_3M_CAM_FLASH_EN);
	gpio_free(GPIO_3M_CAM_FLASH_SET);
	}
...
int32_t msm_sensor_power_up_baffin_duos(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	ret = gpio_request(GPIO_3M_CAM_STBY, "baffin_stby");
	ret = gpio_request(GPIO_3M_CAM_RESET, "baffin_reset");
	ret = gpio_request(GPIO_3M_CAM_A_EN, "baffin_a_en");
	ret = gpio_request(GPIO_3M_CAM_C_EN, "baffin_c_en");
	ret = gpio_request(GPIO_3M_CAM_AF_EN, "baffin_af_en");
	...
	gpio_set_value(GPIO_3M_CAM_A_EN, 1);  // 2 - MAIN VREG_A 2.8 ON
	msleep(1);

   	gpio_set_value(FRONT_CAM_C_EN, 1);  // 3- 2M CAM CORE 1.8V ON
   	gpio_set_value(GPIO_3M_CAM_AF_EN, 1);
	msleep(1);
	printk("BAFFIN_CAM_A_EN = %d , BAFFIN_FRONT_CAM_C_EN = %d \n", gpio_get_value(GPIO_3M_CAM_A_EN), gpio_get_value(FRONT_CAM_C_EN));
	...
	gpio_set_value(GPIO_3M_CAM_C_EN, 1);  // 8 - 5M CAM_C_EN ON 
	msleep(10);

	printk("FRONT_CAM_STBY = %d\n", gpio_get_value(FRONT_CAM_STBY));
	printk("GPIO_3M_CAM_C_EN = %d\n", gpio_get_value(GPIO_3M_CAM_C_EN));
	
	gpio_set_value(GPIO_3M_CAM_STBY, 1); //9 - 5M STBY ON
	printk("GPIO_3M_CAM_STBY = %d\n", gpio_get_value(GPIO_3M_CAM_STBY));	
	msleep(20);

	gpio_set_value(GPIO_3M_CAM_RESET, 1); // 10 - 5M RESET ON
	printk("GPIO_3M_CAM_RESET = %d\n", gpio_get_value(GPIO_3M_CAM_RESET));	
...
int32_t msm_sensor_power_down_baffin_duos(struct msm_sensor_ctrl_t *s_ctrl)
{
	...
	gpio_set_value_cansleep(GPIO_3M_CAM_RESET, 0);	// turn off : 5M CAM_RESET 
	...
	gpio_set_value_cansleep(GPIO_3M_CAM_STBY, 0);		//turn off : 5M STBY
...
#if (defined (CONFIG_MACH_GPIO_3M_OPEN) && !defined(CONFIG_MACH_KYLEPLUS_OPEN)) || defined (CONFIG_MACH_ARUBASLIM_OPEN)
static struct msm_cam_clk_info cam_clk_info[] = {
	{"cam_m_clk", MSM_SENSOR_MCLK_24HZ},
};

int32_t msm_sensor_power_up_jena_open(struct msm_sensor_ctrl_t *s_ctrl)
{
	int32_t rc = 0;
	struct msm_camera_sensor_info *data = s_ctrl->sensordata;
	CDBG("%s: %d\n", __func__, __LINE__);
	...
	rc = gpio_request(GPIO_3M_CAM_STBY, "jena_stby");
	rc = gpio_request(GPIO_3M_CAM_RESET, "jena_reset");
	rc = gpio_request(GPIO_3M_CAM_IO_EN, "jena_a_en");
	rc = gpio_request(GPIO_3M_CAM_C_EN, "jena_c_en");
	rc = gpio_request(GPIO_3M_CAM_AF_EN, "jena_af_en");

	gpio_direction_output(GPIO_3M_CAM_STBY, 0);
	gpio_direction_output(GPIO_3M_CAM_RESET, 0);
	gpio_direction_output(GPIO_3M_CAM_IO_EN, 0);
	gpio_direction_output(GPIO_3M_CAM_C_EN, 0);
	gpio_direction_output(GPIO_3M_CAM_AF_EN, 0);

	gpio_set_value_cansleep(GPIO_3M_CAM_STBY, 0);		//STBYN :: MSM_GPIO 96
	gpio_set_value_cansleep(GPIO_3M_CAM_RESET, 0);	//RSTN :: MSM_GPIO 85
	gpio_set_value_cansleep(GPIO_3M_CAM_IO_EN, 0);
	gpio_set_value_cansleep(GPIO_3M_CAM_C_EN, 0);
	...
	msleep(1);
	gpio_set_value(GPIO_3M_CAM_IO_EN, 1);

//	gpio_tlmm_config(GPIO_CFG(15, 1, 1, 1, 3, 1),GPIO_CFG_ENABLE);
	gpio_tlmm_config(GPIO_CFG(GPIO_3M_CAM_FLASH_SET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
...
int32_t msm_sensor_power_down_jena_open(struct msm_sensor_ctrl_t *s_ctrl)
{
	struct msm_camera_sensor_info *data = s_ctrl->sensordata;
	CDBG("%s\n", __func__);
	if (data->sensor_platform_info->i2c_conf &&
...
#endif //(CONFIG_MACH_GPIO_3M_OPEN) && !(CONFIG_MACH_KYLEPLUS_OPEN)
